...
school project. Its official name is \textit{FileTagger}, but we later changed
the name to \textit{OctoTagger}.
...
interface, and the rest happens automatically in the background. Every tag has
its own folder, and every folder contains links that point to each file with
the according tag.
...
What makes OctoTagger special, is that the entire organization is done via the
regular \emph{file system}. This means that users do not need to open the
application in order to view their files sorted by tags. They can use
their normal file browser, and open and edit the files just like they normally
would. The software only needs to run in order to assign tags.
...
\end{itemize}
\bigskip{}
Wien, am \today % TODO Deutsches today
