...
The original idea was to have a \tfcode{GridSizer} and an appropriate size for
...
See \tfpath{octotagger/src/itemview.py} to follow along in the code while
...
wx.ST_ELLIPSIZE_END} and \tfcode{SetMaxSize((THUMBNAIL_SIZE[0], -1))}). Event
...
calling window, which is always an \tfcode{Item}, takes the \tfcode{Image},
coverts it into a \tfcode{Bitmap} and displays it.
